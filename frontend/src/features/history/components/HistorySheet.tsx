...
export default function HistorySheet({ open, onClose, bookings }: HistorySheetProps) {
  ...
  return (
    <div className="admin-sheet-modal" role="dialog" aria-modal="true">
      <button type="button" className="admin-sheet-modal__backdrop" onClick={onClose} aria-label="Fechar histórico" />
      <section className="admin-sheet-modal__card history-sheet">
        <header className="admin-sheet-modal__header">
          <div>
            <span className="admin-sheet-modal__eyebrow">Admin</span>
            <h3 className="admin-sheet-modal__title">Histórico</h3>
          </div>
          <button type="button" className="admin-sheet-modal__close" onClick={onClose}>×</button>
        </header>

        <div className="history-sheet__list">
          {bookings.length === 0 ? (
        ...
        {activeBooking ? (
          <div className="history-sheet__detail">
            <div className="history-sheet__detail-header">
              <div>
                <span className="admin-sheet-modal__eyebrow">Detalhes</span>
                <h4 className="history-sheet__detail-title">{bookingName(activeBooking)}</h4>
              </div>
              <button type="button" className="admin-sheet-modal__close" onClick={() => setActiveBookingId("")}>×</button>
            </div>
